    ...
    implements BatchImporter
{
    ...
    private final void importBatchInTxn(final NodeRef target,
        ...
        throws InterruptedException,
               OutOfOrderBatchException
    {
    	Boolean useTnx = false;
    	
    	if(useTnx)
    	{
    		
    		//old functionality disabled for SIC
        ...
        if (batch != null)
        {
            for (final BulkImportItem<BulkImportItemVersion> item : batch)
            {
                  if (importStatus.isStopping() || Thread.currentThread().isInterrupted()) {
                    System.out.println("Target - Batchlmporterlmp:  " + target);
                	isStopped = true;
                	//TODO: llamar api del módulo de migración. Actualizar estado Detenido por item con el API
                    System.out.println("||||||||||||||||||||||||||||||||||||||||||||||||||||||");
                    System.out.println("---------------------------Se detuvo la importacion ----------------------");
                    System.out.println("|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
                    Map metadataMap = batch.getContents().get(0).getVersions().first().getMetadata();
                    if(!metadataMap.get(RecursosBundle.getProperty("type.xml")).equals(RecursosBundle.getProperty("descripcion.xml"))){
                        auditoria.auditarEjecucion(
                                (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                                Integer.parseInt(metadataMap.get("idRegistro").toString()),
                                "Detenido por una solicitud de usuario.",
                                7);
                    }

                    if (debug(log)) debug(log, "Ejecución Auditoria: " );
                ...
                else
                {
                    ...
                    if(!metadataMap.get(RecursosBundle.getProperty("type.xml")).equals(RecursosBundle.getProperty("descripcion.xml"))){
                        auditoria.auditarEjecucion(
                                (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                                Integer.parseInt(metadataMap.get("idRegistro").toString()),
                                "Preparando el Documento en el lote de Cargue.",
                                1);
                    }
                    importItem(new NodeRef(item.getVersions().first().getMetadata().get("targetPath").toString()), item, replaceExisting, dryRun);                }
            ...
            if (isStopped){
                ...
                auditoria.auditarEjecucion(
                        ...
                        "Detenido por una solicitud de usuario.",
                        7);
                throw new InterruptedException(Thread.currentThread().getName() + " was interrupted. Terminating early.");
            }
        ...
        throws InterruptedException
    {
        try
        {
            ...
            //jbedoya
            Map metadataMap = item.getVersions().first().getMetadata();
            String targetPath = (String)metadataMap.get("targetPath");
            NodeRef nodeRef = null;
            System.out.println("tarjet - BatchImportlmp:: "+targetPath);
            System.out.println("metataMap - BatchImportlmp:: "+metadataMap.toString());

            auditoria.auditarEjecucion(
                    (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                    Integer.parseInt(metadataMap.get("idRegistro").toString()),
                    "Iniciando Importación del Documento.",
                    2);
            ...
            if(targetPath != null)
            {
                auditoria.auditarEjecucion(
                        (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                        Integer.parseInt(metadataMap.get("idRegistro").toString()),
                        "Iniciando Importación del Documento. Ruta Encontrada en el ECM.",
                        1);
                NodeRef nodeTargetPath = convertPathToNodeRef(serviceRegistry, targetPath.trim());
                nodeRef     = findOrCreateNode(nodeTargetPath, item, replaceExisting, dryRun);
            }
            else
            {
                auditoria.auditarEjecucion(
                        (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                        Integer.parseInt(metadataMap.get("idRegistro").toString()),
                        "Iniciando Importación del Documento. No se ha proporcionado una ruta o esta no coincide con alguna ubicación del arbol documental del ECM, " +
                                "verifique los parametros para generación de ruta de ubicación en la estructura documental del ECM.",
                        6);
            ...
            if (nodeRef != null)
            {
                ...
                else
                {
                    ...
                    if(!metadataMap.get(RecursosBundle.getProperty("type.xml")).equals(RecursosBundle.getProperty("descripcion.xml"))){
                        String uuid = item.getVersions()
                                .first().getMetadata()
                                .get("uuid").toString();
                        Integer idRegistro = Integer.parseInt(item.getVersions()
                                .first().getMetadata()
                                .get("idRegistro").toString());

                        auditoria.auditarEjecucion(Registrar.builder()
                                .uuid(uuid)
                                .mensajeEstadoProceso("Escaneando Archivo: ".concat(idRegistro.toString())
                                        .concat(" en el uudi de importación: ".concat(uuid)))
                                .estadoProceso(1)
            ...
            if (trace(log)) trace(log, "Finalizado Importando " + String.valueOf(item));
            //TODO: Escribir la completitud por registro. Api por registro
            System.out.println("--------------------------------");
            System.out.println("--------------------------------");
            System.out.println("--------------------------------");
            System.out.println("valor del type es "+metadataMap.get(RecursosBundle.getProperty("type.xml")) + "comparando con "+RecursosBundle.getProperty("descripcion.xml"));
            if(!metadataMap.get(RecursosBundle.getProperty("type.xml")).equals(RecursosBundle.getProperty("descripcion.xml"))){
                System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
                System.out.println("----------------- Documento Cargado  ------------------------ ");
                System.out.println("uuid encontrado "+metadataMap.get("uuid"));
                System.out.println("id registro es "+ metadataMap.get("idRegistro") + "transformado : "+Integer.parseInt(metadataMap.get("idRegistro").toString()));
                System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");

                auditoria.auditarEjecucion(
                        (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                        Integer.parseInt(String.valueOf(metadataMap.get("idRegistro"))),
                        "Documento Cargado",
                        5);
                ...
                (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                        Integer.parseInt(metadataMap.get("idRegistro").toString()),
                        "Error al cargar el documento. " + e.getMessage().toString(),
                        6);
                 */
            }

            if (trace(log)) trace(log, "Ejecución Auditoria: ");
        ...
        catch (final Exception e)
        {
            ...
            //TODO: Escribir la completitud por registro. Api por registro
            System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
            System.out.println("----------------- Error Al Cargar Docuemnto  ------------------------ ");
            System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
            System.out.println("Error al cargar documento por "+e.getCause());
            e.printStackTrace();
            Map metadataMap = item.getVersions().first().getMetadata();
                                           ...
                                           final boolean                               dryRun)
    {
        ...
        catch (final OutOfOrderBatchException oobe)
        {
            //TODO: Escribir la completitud por registro. Api por registro
            System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
            System.out.println("----------------- No tiene parent node ref  ------------------------ ");
            System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
            Map metadataMap = item.getVersions().first().getMetadata();
            if(!metadataMap.get(RecursosBundle.getProperty("type.xml")).equals(RecursosBundle.getProperty("descripcion.xml"))){
                auditoria.auditarEjecucion(
    ...
    private NodeRef getParent(final NodeRef target, final BulkImportItem<BulkImportItemVersion> item)
    {
        ...
        if (itemParentPathElements != null && itemParentPathElements.size() > 0)
        {
            ...
            try
            {
                //####_TODO: I THINK THIS WILL FAIL IN THE PRESENCE OF CUSTOM NAMESPACES / PARENT ASSOC QNAMES!!!!
        ...
        throws InterruptedException
    {
        ...
        if (numberOfVersions == 0)
        {
            //TODO: Escribir la completitud por registro. Api por registro
            System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
            System.out.println("----------------- No tiene Version   ------------------------ ");
            System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
            Map metadataMap = item.getVersions().first().getMetadata();
            if(!metadataMap.get(RecursosBundle.getProperty("type.xml")).equals(RecursosBundle.getProperty("descripcion.xml"))){
                auditoria.auditarEjecucion(
        ...
        else
        {
            ...
            for (final BulkImportItemVersion version : item.getVersions())
            {
                ...
                //TODO: Escribir la completitud por registro. Api por registro
                System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
                System.out.println("----------------- Escaneando version  ------------------------ ");
                System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
                Map metadataMap = item.getVersions().first().getMetadata();
                if(!metadataMap.get(RecursosBundle.getProperty("type.xml")).equals(RecursosBundle.getProperty("descripcion.xml"))){
                    auditoria.auditarEjecucion(
        ...
        throws InterruptedException
    {
        ...
        if (version == null)
        {
            ...
            //TODO: Escribir la completitud por registro. Api por registro
            System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
            System.out.println("----------------- la version es null  ------------------------ ");
            System.out.println("||||||||||||||||||||||||||||||||||||||||||||||");
