    ...
    implements BatchImporter
{
    ...
    //Objeto Auditoria para acceder llamar los modulos
        ...
        if (batch != null)
        {
            for (final BulkImportItem<BulkImportItemVersion> item : batch)
            {
                if (importStatus.isStopping() || Thread.currentThread().isInterrupted()) {
                    ...
                    ResponseEntity<?> resutadoAuditoria = auditoria.modificarEstadoEjecucionConRegistro(
                            (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                            "Detenido por una solicitud de usuario.",
                            (metadataMap.get("idRegistro") != null) ? metadataMap.get("idRegistro").toString() : "",
                            7);
                    if (debug(log)) debug(log, "Ejecución Auditoria: " + resutadoAuditoria);
            ...
            if (isStopped) throw new InterruptedException(Thread.currentThread().getName() + " was interrupted. Terminating early.");
            ...
            throws InterruptedException
    {
        try
        {
            ...
            ResponseEntity<?> resutadoAuditoria = auditoria.modificarEstadoEjecucionConRegistro(
                    ...
                    (metadataMap.get("idRegistro") != null) ? metadataMap.get("idRegistro").toString() : "",
                    5);
            if (trace(log)) trace(log, "Ejecución Auditoria: " + resutadoAuditoria);
