  ...
  /*  @Override
    ...
    public void run()
    {
        ...
        try
        {
            ...
            // Phase 2 - File scanning
            // ------------------------------------------------------------------
            //TODO: llamar api de modulo migracion. En este se deberá actualizar todos los registros por uuid

            Map metadataMap = currentBatch.get(0).getVersions().first().getMetadata();
            ResponseEntity<?> resutadoAuditoria;

            resutadoAuditoria = auditoria.modificarEstadoEjecucion(
                    (metadataMap.get("uuid") != null) ? metadataMap.get("uuid").toString() : "",
                    "Escaneando Expedientes y Archivos",
                    1);
        ...
        catch (final Throwable t)
        {
            ...
            {
                // A stop import was requested
                if (debug(log)) debug(log, Thread.currentThread().getName() + " fue detenido por una solicitud de detención.", t);
            ...
            {
                // An unexpected exception occurred during scanning - log it and kill the import
                error(log, "Falló ejecución desde '" + source.getName() + "'.", t);

                Map metadataMap = currentBatch.get(0).getVersions().first().getMetadata();
                ResponseEntity<?> resutadoAuditoria = auditoria.modificarEstadoEjecucion(
                        metadataMap.get("uuid").toString(),
                        "Falló ejecución desde '" + source.getName() + "'.",
                        6);
                if (debug(log)) debug(log, "Ejecución Auditoria: " + resutadoAuditoria);
                //TODO: llamar api de modulo migracion. En este se deberá actualizar todos los registros por uuid
                importStatus.unexpectedError(t);
